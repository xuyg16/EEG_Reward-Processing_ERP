...
*Refer to: `s11a_stats_prep.py`, `s11b_stats.py`, `s12_permutation_bootstrap.py`*
...
**Original Pipeline** Original applied paired t-tests to RewP data, comparing the Mid-High and High-High conditions. A one-way repeated-measures ANOVA was used on behavioral data to examine win rates across tasks (Low, Mid, High), and a paired t-test was used to compare performance, which is defined as the proportion of correct arm selections, between the Mid- and High-value tasks.

**Proposed Pipeline** We replicated the original statistical tests and additionally applied permutation t-tests to mitigate the reduced statistical power resulted from our smaller sample size.