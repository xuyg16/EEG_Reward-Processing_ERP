...
from scripts.pipeline.epoch_io import load_epochs


def decode_context(epochs: mne.Epochs, context: str, n_splits: int = 5):
    # Select epochs for the given context and prepare data and labels
    ...
    # Limit the number of CV splits to the number of trials in the smallest class to avoid errors during cross-validation
    cv_splits = min(n_splits, min_class_n)
    X = context_epochs.get_data(copy=True) # shape (n_trials, n_channels, n_times)

    # Set up the decoding pipeline and cross-validation
...
def summarize_subject(subject_id: str, context: str, result: dict, window_start: float, window_end: float) -> dict:
    ...
    # find peak decoding time across the whole epoch,
    # mean_scores is the average AUC across CV folds for each time point, 
    # so we take the argmax to find the time point with the highest mean AUC
...
def compute_group_stats(summary_df: pd.DataFrame, contexts: list[str]) -> dict:
    ...
    # Compute mean and std of window AUC for each context
    ...
    # Perform a paired t-test on the window AUCs across subjects
    if len(contexts) == 2:
...
def save_outputs(output_dir: Path, pipeline_name: str, contexts: list[str],
                 summary_df: pd.DataFrame, group_stats: dict, timecourse_store: dict):
...
def run_group_decoding(subjects: list[str], pipeline_name: str, contexts: list[str],
                       window_start: float = 0.2, window_end: float = 0.35,
                       logger=None):
    summary_rows = []
    timecourse_store = {}
    ...
    for subject_id in subjects:
        ...
        for context in contexts:
            ...
            if logger is not None:
                logger.info(
                    "sub-%s %s: n=%s win=%s loss=%s window_auc=%.4f",
                    subject_id,
                    context,
                    result["n_trials"],
                    result["n_win"],
                    result["n_loss"],
                    float(result["mean_scores"][(result["times"] >= window_start) & (result["times"] <= window_end)].mean()),
                )
            summary_rows.append(
