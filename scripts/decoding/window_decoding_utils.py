...
def decode_context_window(
    ...
    scores = cross_val_score(estimator, X.reshape(len(X), -1), y, cv=cv, scoring="roc_auc", n_jobs=1)
...
def run_group_decoding_window(
    ...
    for subject_id in subjects:
        ...
        for context in contexts:
            ...
            if logger is not None:
                logger.info(
                    "sub-%s %s: n=%s win=%s loss=%s mean_auc=%.4f",
                    subject_id,
                    context,
                    result["n_trials"],
                    result["n_win"],
            ...
            summary_rows.append(
                summarize_subject_window(subject_id, context, result)
            )
